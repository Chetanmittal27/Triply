...
const buildDays = (start, end) => {
    const days = [], cursor = new Date(start), last = new Date(end);
    cursor.setUTCHours(0, 0, 0, 0); last.setUTCHours(0, 0, 0, 0);
    if (cursor > last) throw new ApiError(400, "Invalid trip dates");
    for (let dayNumber = 1; cursor <= last; dayNumber += 1) { days.push({ dayNumber, onDate: new Date(cursor) }); cursor.setUTCDate(cursor.getUTCDate() + 1); }
    return days;
};
...
export const createItinerary = asyncHandler(async (req, res) => {
    ...
    await Itinerary.bulkWrite(days.map((day) => ({ updateOne: { filter: { tripId: trip._id, dayNumber: day.dayNumber }, update: { $setOnInsert: { ...day, tripId: trip._id } }, upsert: true } })));
...
export const getItinerary = asyncHandler(async (req, res) => {
    ...
    const itinerary = await Itinerary.find({ tripId: req.params.id }).sort({ dayNumber: 1 }).lean();
    const activities = await Activity.find({ itineraryId: { $in: itinerary.map((day) => day._id) } }).sort({ onTime: 1 }).lean();
    const result = itinerary.map((day) => ({ ...day, activities: activities.filter((activity) => String(activity.itineraryId) === String(day._id)) }));
...
export const updateItinerary = asyncHandler(async (req, res) => {
    ...
    const itinerary = await Itinerary.findOne({ _id: req.params.itineraryId, tripId: req.params.tripId });
    ...
    await itinerary.save(); res.json(new ApiResponse(200, itinerary, "Itinerary updated successfully"));
});
...
export const deleteItinerary = asyncHandler(async (req, res) => {
    ...
    const itinerary = await Itinerary.findOne({ _id: req.params.itineraryId, tripId: req.params.tripId });
    if (!itinerary) throw new ApiError(404, "Itinerary day not found");
    await Activity.deleteMany({ itineraryId: itinerary._id }); await itinerary.deleteOne();
...
const requestAiPlan = async (trip, preferences) => {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401) throw new ApiError(401, "AI provider rejected the API key. Check OPENROUTER_API_KEY.");
        if (response.status === 429) throw new ApiError(429, "AI provider rate limit reached. Try again in a minute.");
        throw new ApiError(502, payload.error?.message || "AI provider request failed");
    }
    const text = payload.choices?.[0]?.message?.content;
    if (!text) throw new ApiError(502, "AI provider returned no itinerary");
    let plan;
    try { plan = JSON.parse(text.replace(/^```json\s*|\s*```$/g, "").trim()); }
    catch { throw new ApiError(502, "AI returned an invalid itinerary. Try generating again."); }
    if (!Array.isArray(plan?.days) || plan.days.length === 0) {
        throw new ApiError(502, "AI returned an unexpected format. Try generating again.");
    }
    return plan;
};
const saveAiPlan = async (trip, plan, userId, replace) => {
    const itinerary = await Itinerary.find({ tripId: trip._id }).sort({ dayNumber: 1 });
    if (itinerary.length === 0) { const days = buildDays(trip.startDate, trip.endDate); await Itinerary.insertMany(days.map((day) => ({ ...day, tripId: trip._id }))); return saveAiPlan(trip, plan, userId, replace); }
    if (replace) await Activity.deleteMany({ itineraryId: { $in: itinerary.map((day) => day._id) } });
    const byDay = new Map(itinerary.map((day) => [day.dayNumber, day]));
    const inserts = (plan.days || []).flatMap((day) => (day.activities || []).map((activity) => ({ itineraryId: byDay.get(day.dayNumber)?._id, title: activity.title, desc: activity.description || "", location: activity.location || trip.destination, onTime: activity.time || "09:00", type: ["Sightseeing", "Food", "Transport", "Hotel", "Adventure", "Other"].includes(activity.type) ? activity.type : "Other", updatedBy: userId }))).filter((activity) => activity.itineraryId && activity.title);
    if (inserts.length) await Activity.insertMany(inserts);
    return itinerary;
};

const generateAi = (replace) => asyncHandler(async (req, res) => {
    const trip = await getTripForMember(req.params.id, req.user._id);
    const plan = await requestAiPlan(trip, req.body);
    const itinerary = await saveAiPlan(trip, plan, req.user._id, replace);
    res.json(new ApiResponse(200, { itinerary, plan }, replace ? "Itinerary regenerated successfully" : "Itinerary generated successfully"));
});
