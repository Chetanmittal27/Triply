...
const verifyEmail = asyncHandler(async(req , res) => {
    ...
    // Bug fix: this used to redirect to `/login?verified=success`, a route
    // the SPA never checks for (it only special-cases the `/email-verified`
    // path). That meant a successful verification silently dropped the user
    // on the plain sign-in screen with zero confirmation that anything
    // worked. Every outcome now consistently lands on `/email-verified`.
...
const updateProfile = asyncHandler(async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(req.user._id, update, { returnDocument: "after", runValidators: true }).select("-password -refreshToken -emailVerificationToken -emailVerificationExpires -passwordResetToken -passwordResetExpires");
