...
export const inviteMember = asyncHandler(async (req, res) => {
    ...
    await trip.save();
    if (user.email) {
        const tripUrl = `${process.env.FRONTEND_URL || "http://localhost:5173"}`;
        sendEmail({
            to: user.email,
            subject: `You've been added to ${trip.title} on Triply`,
            html: `<h2>${trip.title}</h2><p>${req.user.fullName || req.user.username} added you to a trip to ${trip.destination}.</p><p>Open Triply to view the itinerary and collaborate: <a href="${tripUrl}">${tripUrl}</a></p>`,
        }).catch((error) => console.error("Trip invitation email failed:", error.message));
    }
